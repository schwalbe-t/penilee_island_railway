...
class Shader {
    ...
    constructor(vertexSrc: String, fragmentSrc: String) {
        val vertId = this.compileShader(
            GL_VERTEX_SHADER, "Vertex", vertexSrc
        )
        val fragId = this.compileShader(
            GL_FRAGMENT_SHADER, "Fragment", fragmentSrc
        )
        ...
        if(glGetProgrami(this.id, GL_LINK_STATUS) == GL_FALSE) {
            ...
            throw RuntimeException("Shader failed to link:\n${infoLog}")
        }
    ...
    private fun compileShader(
        glType: Int, dispType: String, source: String
    ): Int {
        ...
        if(glGetShaderi(id, GL_COMPILE_STATUS) == GL_FALSE) {
            ...
            glDeleteShader(id)
            throw RuntimeException("${dispType} shader failed to compile:\n${infoLog}")
        }
        return id
    }

    inline private fun findUniform(name: String, f: (Int) -> Unit) {
        this.bindRaw()
    ...
    fun setTexture2(n: String, v: Texture2) = this.findUniform(n) { l ->
        val slot = this.allocateTextureSlot(n, v.id, GL_TEXTURE_2D)
        glActiveTexture(GL_TEXTURE0 + slot)
        glBindTexture(GL_TEXTURE_2D, v.id)
        glUniform1i(l, slot)
    }
    fun setTexture3(n: String, v: Texture3) = this.findUniform(n) { l ->
    ...
    fun setFloat(n: String, v: Float)
        = this.findUniform(n) { l -> glUniform1f(l, v) }
    fun setVec2(n: String, v: Vector2f)
        = this.findUniform(n) { l -> glUniform2f(l, v.x(), v.y()) }
    fun setVec3(n: String, v: Vector3f)
        = this.findUniform(n) { l -> glUniform3f(l, v.x(), v.y(), v.z()) }
    fun setVec4(n: String, v: Vector4f) = this.findUniform(n) { l -> 
        glUniform4f(l, v.x(), v.y(), v.z(), v.w()) 
    }
    fun setMatrix2(n: String, v: Matrix2f) = this.findUniform(n) { l ->
    ...
    fun setMatrix3(n: String, v: Matrix3f) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setMatrix4(n: String, v: Matrix4f) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setFloatArr(n: String, v: List<Float>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setVec2Arr(n: String, v: List<Vector2f>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setVec3Arr(n: String, v: List<Vector3f>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setVec4Arr(n: String, v: List<Vector4f>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setMatrix2Arr(n: String, v: List<Matrix2f>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setMatrix3Arr(n: String, v: List<Matrix3f>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setMatrix4Arr(n: String, v: List<Matrix4f>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
    ...
    fun setInt(n: String, v: Int)
        = this.findUniform(n) { l -> glUniform1i(l, v) }
    fun setIntArr(n: String, v: List<Int>) = this.findUniform(n) { l -> 
        MemoryStack.stackPush().use { stack ->
    ...
    fun setUInt(n: String, v: UInt) = this.findUniform(n) { l ->
        glUniform1ui(l, v.toInt())
    }
    fun setUIntArr(n: String, v: List<UInt>) = this.findUniform(n) { l ->
        MemoryStack.stackPush().use { stack ->
            val buff = stack.mallocInt(v.size)
